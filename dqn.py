...
checkpoint_dir = "checkpoints-plotting-graphs-1/"
state_size = 3
state_shape = [state_size]
total_num_actions = 2 # Only 2 actions Buy and Sell as mentioned by Jonah Varon and Anthony Sorokoa
...
class EpsilonGreedy:
    ...
    def get_action(self, q_values, iteration, training_action):
        """
...
class NeuralNetwork:
    ...
    def __init__(self, num_actions, replay_memory):
        ...
        with pt.defaults_scope(activation_fn=tf.nn.relu):
            self.q_values = x_pretty. \
                fully_connected(size=5, name='layer_fc1', weights=init). \
                fully_connected(size=5, name='layer_fc3', weights=init). \
        ...
        # self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=self.q_values, labels=self.q_values_new))
    ...
    def optimize(self, min_epochs=1.0, max_epochs=10,
        ...
        for i in range(max_iterations):
            ...
            # print("Loss ", loss_per_optimization)
...
class ReplayMemory:
    ...
    def prepare_sampling_prob(self, batch_size=BATCH_SIZE):
        ...
        # Create an index of the estimation errors that are high.
        self.idx_err_hi = np.squeeze(np.where(np.logical_not(idx)))
        # print ("sizes ", self.idx_err_lo.size, " ", self.idx_err_hi.size, " ",self.idx_err_hi)
        # Probability of sampling Q-values with high estimation errors.
        # This is either set to the fraction of the replay-memory that
        # has high estimation errors - or it is set to 0.5. So at least
        # half of the batch has high estimation errors.
    ...
    def random_batch(self):
        ...
        # print ("self.idx_err_hi ", self.idx_err_hi, "self.num_samples_err_hi ", self.num_samples_err_hi)
        ...
        # print (idx_hi, " ", idx_lo)
        idx = random.sample(range(0, self.num_used), BATCH_SIZE) #Generating random batch from sample
...
class Agent:
    ...
    def get_value_of_index(self,count):
        data_index = (count % Xs.shape[0])
        return Xs[data_index][0]

    def get_state(self, count):
        return X[None,(count % X.shape[0])] #['Open','High','Low', 'HL_PCT','PCT_change','Close', 'Total Trade Quantity']
    ...
    def get_portfolio_value(self,index,action):
        if action == 0:
            self.portfolio_value = self.portfolio_value - self.get_value_of_index(index)
        else:
            self.portfolio_value = self.portfolio_value + self.get_value_of_index(index)

    def get_reward(self, action_taken, curr_index, initial_action_value):
        if self.first_action == action_taken:
             calculated_reward = 0
        elif action_taken == 0:
            self.get_portfolio_value(curr_index,action_taken)
            # print("Current value ", self.get_value_of_index(curr_index), "Initial value ", initial_action_value," Reward",(self.get_value_of_index(curr_index) - initial_action_value), "Current action 0", "First action ", self.first_action)
            self.first_action = 4
            calculated_reward = (self.get_value_of_index(curr_index) - initial_action_value) / initial_action_value

        else:
            # print("Current value ", self.get_value_of_index(curr_index), "Initial value ", initial_action_value,
            #       " Reward", (self.get_value_of_index(curr_index) - initial_action_value), "Current action 1",
            #       "First action ", self.first_action)
            self.get_portfolio_value(curr_index, action_taken)
            self.first_action = 4
            calculated_reward = -1 * (self.get_value_of_index(curr_index) - initial_action_value) / initial_action_value
        return calculated_reward * 10
    ...
    def plot_graphs(self):
        ...
        plt.show()
        plt.plot(self.plot_reward_over_number_of_iterations)
        plt.title("Reward over number of iterations")
        plt.xlabel("Number of iterations")
        plt.ylabel("Reward")
        plt.show()
        plt.plot(self.plot_portfolio_over_number_of_episodes)
        plt.title("Portfolio over number of episodes")
        plt.xlabel("Number of episodes")
        plt.ylabel("portfolio value")
        plt.show()
    ...
    def run(self, num_episodes=None):
        ...
        max_episode_size = X.shape[0] #Max size of any episode
        ...
        while count_episodes <= num_episodes:
            if end_episode:
                ...
                state = self.get_state(count_states)
                episode_size = 0
                self.first_action = 4
                ...
                initial_portfolio_value = self.portfolio_value = 100000
            ...
            if self.first_action == 4:
                self.first_action = action
                self.first_action_value = self.get_value_of_index(count_states) # Getting the value for first action to calculate reward at later point
            ...
            reward = self.get_reward(action_taken=action, curr_index=count_states,initial_action_value=self.first_action_value) # Get the reward
            ...
            if episode_size > max_episode_size:
                ...
                if reward == 0:
                    if self.first_action == 0:
                        reward = self.get_reward(1,count_states,self.first_action_value)
                    else:
                        reward = self.get_reward(0,count_states,self.first_action_value)
            ...
            if self.portfolio_value < 50000:
                reward = -1
                end_episode = True
            elif self.portfolio_value > 120000:
                reward += 1
            ...
            if self.training:
                ...
                self.replay_memory.add(state=state,
                                       ...
                                       action=action,
                                       reward=reward,
                                       end_episode=end_episode) # Should I add state t+1 (next state)
...
if __name__ == '__main__':
    ...
    desc = "Reinformenct Learning (Q-learning) for Stock trading agent."
